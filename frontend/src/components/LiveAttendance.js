...
const LiveAttendance = () => {
  ...
  const handleScanImage = async () => {
    ...
    try {
      ...
      // Filter out unrecognized faces locally
      const recognizedMatches = matches.filter(m => m.roll !== "Unknown" && m.name !== "Unknown");
      const imageStudents = recognizedMatches.map(m => m.roll);

      // Update Attendees with Source Tracking
      setAttendees(prev => {
        const newAttendees = [...prev];
        recognizedMatches.forEach(match => {
      ...
      const unrecognizedCount = matches.length - recognizedMatches.length;
      let msg = `Scanned successfully! Identified ${recognizedMatches.length} students.`;

      if (unrecognizedCount > 0) {
        msg += `\n(${unrecognizedCount} faces were detected but not recognized).`;
      }

      if (debug) {
        msg += `\n\n(Debug: Detected ${debug.faces_detected} faces total)`;
        if (debug.faces_detected > 0 && recognizedMatches.length === 0) {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const finalFilename = `${cleanSubject}_${cleanSection}_${cleanTime}.csv`;
      const finalHour = finalTime ? finalTime.split(':')[0] : '1';

      // 2. Save attendance records to MongoDB via /api/attendance/live
      ...
      // Add metadata
      attendanceFormData.append('course', finalSubject || 'Unknown Subject');
      attendanceFormData.append('class', className);
      attendanceFormData.append('hour', finalHour);
      attendanceFormData.append('report_type', 'both');

      // NEW: Send current attendees roll numbers to sync manually marked students
      const presentRolls = attendees.map(a => a.roll);
      attendanceFormData.append('present_rolls', JSON.stringify(presentRolls));

      // Call the backend to save attendance records
      await takeLiveAttendance(attendanceFormData);

      // 3. Save Excel file to GridFS
      const fileBlob = generateCSV();
      const excelFormData = new FormData();
      excelFormData.append('file', fileBlob, finalFilename);
      excelFormData.append('course', finalSubject || 'Unknown Subject');
      excelFormData.append('class', className);
      excelFormData.append('hour', finalHour);
      excelFormData.append('type', 'Lecture'); // Default to Lecture for auto-save
  ...
  const handleManualOverrideSubmit = async ({ students, reason }) => {
    ...
    try {
      ...
      await submitManualOverride({
        ...
        period: classTime ? classTime.split(':')[0] : '1',
        className,
  ...
  return (
    <div style={{ background: 'var(--bg-primary)', minHeight: '100vh', display: 'flex', flexDirection: 'column', transition: 'background var(--transition-theme)' }}>
      ...
      {/* Save Modal Removed - Auto-saving enabled */}
