...
def live_attendance():
    ...
    # NEW: Check for explicitly provided present roll numbers from frontend
    manual_present_rolls = request.form.get("present_rolls")
    if manual_present_rolls:
        import json
        try:
            roll_list = json.loads(manual_present_rolls)
            for roll in roll_list:
                # Get student info for these rolls
                student_doc = students_col.find_one({"rollNo": str(roll)})
                if student_doc:
                    attendance[str(roll)] = {
            ...
            print(f"Error parsing manual_present_rolls: {e}")

    # Process images only if no manual rolls or in addition to them
    if not attendance and 'images' in request.files:
